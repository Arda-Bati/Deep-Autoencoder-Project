...
function [ scores ] = pesqbin_linux( reference, degraded, fs, mode, binary )
    ...
    if isstr(reference)
        ...
        audiowrite( file.reference, 0.999*reference./max(abs(reference)), ...
                    fs, 'BitsPerSample', nbits );
    end % isstr(reference)
    ...
    if isstr(degraded)
        ...
        audiowrite( file.degraded, 0.999*degraded./max(abs(degraded)), ...
                    fs, 'BitsPerSample', nbits );
